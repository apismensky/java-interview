 ...
 * ss also satisfies the same. e.g. "\text{catsanddog}catsanddog" can be split into two substrings "\text{catsand}catsand", "\text{dog}dog".
 * The subproblem "\text{catsand}catsand" can be further divided into "\text{cats}cats","\text{and}and",
 * which individually are a part of the dictionary making "\text{catsand}catsand" satisfy the condition.
 * Going further backwards, "\text{catsand}catsand", "\text{dog}dog" also satisfy the required criteria individually
 ...
 *  For every such substring, we partition the string into two further substrings s1's1
 * ′
 *   and s2's2
 * ′
 *   in all possible ways using the index jj (Note that the ii now refers to the ending index of s2's2
 * ′
 *  ). Now, to fill in the entry \text{dp}[i]dp[i], we check if the \text{dp}[j]dp[j] contains \text{true}true, i.e.
 *  if the substring s1's1
 * ′
 *   fulfills the required criteria. If so, we further check if s2's2
 * ′
 *   is present in the dictionary. If both the strings fulfill the criteria,
 *   we make \text{dp}[i]dp[i] as \text{true}true, otherwise as \text{false}false.
 */

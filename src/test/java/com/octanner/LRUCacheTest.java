...
public class LRUCacheTest {
    ...
    public void test3() {
        ...
        validate("LRU", cache.lruNode, newArrayList(2));
        validate("MRU", cache.mruNode, newArrayList(2));
        assertEquals(1, (int)cache.get(2));
        validate("LRU", cache.lruNode, newArrayList(2));
        validate("MRU", cache.mruNode, newArrayList(2));
        cache.put(3, 2);
        assertNull(cache.get(2));
        assertEquals(2, (int)cache.get(3));
    }
    ...
    @Test
    public void test7() {
        cache = new LRUCache<Integer, Integer>(4);
        cache.put(1, 1);
        cache.put(2, 2);
        cache.put(3, 3);
        cache.put(4, 4);
    ...
    public void test9() {
        ...
        assertNull(cache.get(1));
        assertEquals(3, (int)cache.get(2));
    }
    ...
    public void test11() {
        ...
        validate("MRU", cache.mruNode, newArrayList(1, 2));
        assertEquals(2, (int)cache.get(1));
        assertEquals(6, (int)cache.get(2));
    }

    private void validate(String label, 
                          LRUCache.CacheNode<Integer, 
                                  Integer> node, List<Integer> expectedKeys) {
        System.out.print(label + ": ");
        Set<LRUCache.CacheNode<Integer, Integer>> visited = new HashSet<LRUCache.CacheNode<Integer, Integer>>();
        int count = 0;
        LRUCache.CacheNode<Integer, Integer> current = node;
        while (current != null) {
